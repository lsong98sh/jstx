	...
	*/
	...
	let directives = [
		...
		},
		{
			...
			compile: function(jst, node, opt, action, code) {
				if (action.value != null) {
					code.push(`jst.fx.slot.render($jst, $ctl, $jst.slots[${attr2text(action.value)}]);`, action.tabs);
				}
				return code;
			},
		},
	...
	function jst(template, target, option, parent) {
		let awaits = [], self = this, prop = Object.defineProperty;
		prop(self, "settings", {
			value: parent? parent.settings : {}
		});
				
		self.addAwait = function(promise) {
			awaits.push(promise);
		}
		// 预处理
		...
		self.ctx = null;
		self.ready = Promise.all(awaits).then(awaits = []);
	...
	jst.prototype.render = function(data, force) {
		...
		return this.ready.then(function(){
			if (self.template.dnt.length > 0 && (force || !self.ctx)) {
	...
	jst.prototype.refresh = function(force){
		return this.render(this.data, force);
	};
	...
	jst.fx = {
		...
		slot: {
			...
			render: function(parent, ctl, slot){
				...
				let sjst = jst.fx.data(ctl, '$slot-jst');
				if(refresh) {
					let dom = document.createElement("div");
					dom.innerHTML = slot;
					sjst = new jst(dom, ctl);
					jst.fx.data(ctl, '$slot-jst',sjst);
				}
				sjst.render(data, refresh);
			}
		},
		include: {
			...
			set: function(parent, ctl, name, value){
				let t = jst.fx.data(ctl, '$include', jst.fx.include.data(), true);
				t[name] = value;
			},
			...
			render: function(parent, ctl){
				...
				if(t.pre != t.name ){
					if(t.jst) {
						let i = parent.children.indexOf(t.jst);
						if (i > -1) {
	...
	jst.prototype.on = function(name, callback){
		let obj = this;
		this.target.addEventListener(name, function(){
	...
	jst.event = {
		listener : function(event){
			...
			if(dom.jst_listeners[e.type]){
				dom.jst_listeners[e.type].forEach(function(item){
					...
					if(doms.length){
						...
						for(let i = 0; i < path.length; ++ i){
							for(let n = 0; n < doms.length; ++ n){
								if(doms[n] == path[i]){
									item.callback.apply(doms[n], arguments);
									return;
								}
							}
						}
		...
		off: function(dom, name, selector, callback){
			if(!dom.jst_listeners){
				return;
			}
			if(arguments.length == 1){
				for(e in dom.jst_listeners){
					dom.removeEventListener(e, jst.event.listener);
				}
				delete dom.jst_listeners;
				return;
			}
			if(!dom.jst_listeners[name]){
				return;
			}
			if(arguments.length == 2){
				dom.removeEventListener(name, jst.event.listener);
				delete dom.jst_listeners[name];
				return;
			}
			dom.jst_listeners[name] = dom.jst_listeners[name].filter(function(item){
				if(callback){
					return !(item.selector != selector && callback == item.callback);
				}else {
					return item.selector != selector;
				}
			});
			if(dom.jst_listeners[name].length == 0){
				dom.removeEventListener(name, jst.event.listener);
				delete dom.jst_listeners[name];
			}
		},
		on: function(dom, name, selector, callback){
			if(!dom.jst_listeners){
				dom.jst_listeners = {};
			}
			if(dom.jst_listeners[name]){
				dom.jst_listeners[name].push({"selector": selector, "callback": callback});
			}
			dom.jst_listeners[name] = [{"selector": selector, "callback": callback}];
			dom.addEventListener(name, jst.event.listener);
		},
